...
theme.tasklist_fg_focus                         = "#3992af"
...
theme.mem 					= theme.icon_dir .. "/indicator-sensors-memory.svg"
theme.temp					= theme.icon_dir .. "/indicator-sensors-cpu.svg"
...
theme.bat_ac 					= theme.icon_dir .. "/battery-caution-charging.svg"
theme.bat_ok 					= theme.icon_dir .. "/battery-medium.svg"
theme.bat_low					= theme.icon_dir .. "/battery-low.svg"
theme.bat_empty					= theme.icon_dir .. "/battery-empty.svg"
...
local blue   = "#80CCE6"
local space3 = markup.font("Hack Nerd Font 3", " ")

-- Clock
local mytextclock = wibox.widget.textclock(markup("#FFFFFF", space3 .. "%H:%M   " .. markup.font("Hack Nerd Font 4", " ")))
...
-- Calendar
local mytextcalendar = wibox.widget.textclock(markup.fontfg(theme.font, "#FFFFFF", space3 .. "%d %b " .. markup.font("Hack Nerd Font 5", " ")))
...
-- Meshle Mail IMAP check
theme.meshle = lain.widget.imap({
    timeout  = 180,
    server   = "imap.meshle.com",
    mail     = "famous.jingo@meshle.com",
    is_plain = false,
    pwdtimeout = 180,
    password = function()
	cmd = io.popen('gpg --quiet --no-tty --decrypt ~/.config/neomutt/passwords/meshle-pass.gpg', 'r')
	out = cmd:read('*a')
...
-- Gmail Mail IMAP check
theme.gmail = lain.widget.imap({
    timeout  = 180,
    server   = "imap.gmail.com",
    mail     = "jfamousket@gmail.com",
    is_plain = false,
    pwdtimeout = 180,
    password = function()
	cmd = io.popen('gpg --quiet --no-tty --decrypt ~/.config/neomutt/passwords/account.gpg', 'r')
	out = cmd:read('*a')
	pass = string.gsub(out, '[\n\r]+', '')
	return pass
    end,
...
-- HSRW Mail IMAP check
theme.hsrw = lain.widget.imap({
    timeout  = 180,
    server   = "mail-kamp.hsrw.org",
    mail     = "25873@hsrw.org",
    is_plain = false,
    pwdtimeout = 180,
    password = function()
	cmd = io.popen('gpg --quiet --no-tty --decrypt ~/.config/neomutt/passwords/hsrw-pass.gpg', 'r')
	out = cmd:read('*a')
	pass = string.gsub(out, '[\n\r]+', '')
	return pass
    end,
...
-- Coretemp
local tempicon = wibox.widget.imagebox(theme.temp)
...
theme.mpd = lain.widget.mpd({
    settings = function ()
        if mpd_now.state == "play" then
            ...
            widget:set_markup(markup.font("Hack Nerd Font 4", " ")
                              .. markup.font(theme.taglist_font,
                              " " .. mpd_now.artist
                              .. " - " ..
                              mpd_now.title .. "  ") .. markup.font("Hack Nerd Font 5", " "))
            play_pause_icon:set_image(theme.pause)
        elseif mpd_now.state == "pause" then
            widget:set_markup(markup.font("Hack Nerd Font 4", " ") ..
                              markup.font(theme.taglist_font, " MPD PAUSED  ") ..
                              markup.font("Hack Nerd Font 5", " "))
...
local net = lain.widget.net({
    settings = function()
        widget:set_markup(markup.font("Hack Nerd Font 1", " ") .. markup.font(theme.font, net_now.received .. " - "
                          .. net_now.sent) .. markup.font("Hack Nerd Font 2", " "))
    end
})
...
theme.weather = lain.widget.weather({
    city_id = 2934691, -- placeholder (London)
    notification_preset = { font = "Hack Nerd Font 9", position = "bottom_right" },
})
...
-- Separators
local first = wibox.widget.textbox('<span font="Hack Nerd Font 7"> </span>')
