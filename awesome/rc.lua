...
awful.util.tagnames = { " SFW ", " WWW ", " DEV ", " SYS ", " CHAT ", " ENT ", " SCHL " }
...
globalkeys = my_table.join(
    -- Take a screenshot
    awful.key({ altkey }, "p", function() os.execute("maim -s | tee  ~/screenshots/$(date +%s).png | xclip -selection clipboard -t image/png") end,
              {description = "take a screenshot", group = "hotkeys"}),

	-- Spotify Controls
	awful.key({ XF86WakeUp }, "End", function () os.execute("sp next") end, {
			description = "spotify next", group = "hotkeys"}),
	awful.key({ XF86WakeUp }, "Home", function () os.execute("sp prev") end, {
			description = "spotify prev", group = "hotkeys"}),
	awful.key({ }, "Pause", function () os.execute("sp play") end, {
			description = "spotify play", group = "hotkeys"}),
	--[[awful.key({ XF86WakeUp }, "Up", function () os.execute("sp stop") end, {
			description = "spotify stop", group = "hotkeys"}),
	--]]
    -- X screen locker
    awful.key({}, "XF86Favorites",  function () os.execute(scrlocker) end,
   ...
   awful.key({ modkey }, "p", function() xrandr.xrandr() end),
    ...
    awful.key({ modkey, "Shift"   }, "Return", function () awful.layout.inc(1)                end,
              {description = "next client layout", group = "layout"}),
    awful.key({ modkey, "Shift"   }, "BackSpace", function () awful.layout.inc(-1)                end,
              {description = "previous client layout", group = "layout"}),
    ...
    awful.key({}, "XF86AudioMicMute", function () os.execute(string.format("amixer -q set 'Capture' toggle")) end, {description = "toggle mic on", group = "hotkeys"}),
    awful.key({}, "XF86AudioMute",
        function ()
            os.execute(string.format("amixer sset Master toggle", beautiful.volume.togglechannel or beautiful.volume.channel))
            beautiful.volume.notify()
        end,
...
awful.rules.rules = {
    ...
    -- Set vs code to always map on the third tag on screen 1.
    { rule = { class = "Code - Insiders" },
      properties = { screen = 1, tag = awful.util.tagnames[3] } },
      
    -- Set reactotron to always map on the first tag on screen 1.
